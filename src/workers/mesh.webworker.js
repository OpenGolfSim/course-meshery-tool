...
import { distanceToPolygonEdge, isPointInPolygon } from '../lib/mesh.js';
import { generateCoursePolygons } from './utils/svg';
...
function conformTerrain({
  ...
  console.log('conformTerrain', settings);
...
function smoothTerrain({ settings, project }) {
  const { terrainSmoothingRadius } = settings;
  return smoothTerrainData(project._heightMap.data, project._heightMap.size, terrainSmoothingRadius);
}
